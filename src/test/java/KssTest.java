...
public class KssTest {
    ...
    public void testSplit() {
        ...
        List<String> splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(3);
    }

    @Test
    public void testSingleQuotes() {
        String text = "여당이 내놓은 상가건물 임대차보호법 개정안, 이른바 ‘임대료 멈춤법’에 대한 논의가 급물살을 타면서 자영업자들과 임대인의 의견이 팽팽하게 맞서고 있다. 신종 코로나바이러스 감염증(코로나19) 확진자 급증세로 집합 제한·집합 금지 기간이 길어지면서 한계에 직면한 소상공인과 자영업자들 사이에서는 임대료 부담을 호소하며 \"법안을 조속히 시행해 달라\"는 목소리가 터져나오고 있다. 반면 임대인들은 임대료 인하를 강제화하는 것은 부당하다며 정부와 여당이 ‘나쁜 임대인(건물주)’ 프레임을 만들고 있다고 비판했다. 업계 전문가들 사이에서도 우려의 목소리가 여럿 있다. 재산권 침해 소지가 있고, 월세 수익이 끊기면 생활이 곤란해지는 ‘생계형 임대인’들이 피해를 볼 수 있는 등 또 다른 부작용이 발생할 수 있다는 것이다. 법 개정 자체만으로 상가 거래 시장이 위축될 가능성도 지적됐다.";
        List<String> splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(6);
    }

    @Test
    public void testQuoteMisalignment() {
        String text = "부부 싸움 규칙 가운데 ‘돈 히트 언더 더 벨트’(Don’t hit under the belt)가 있다. 권투할 때 벨트 아래를 치면 반칙이듯이, 상대가 너무 아파할 만한 것을 건드리면 회복하기 어렵다. 그 부분은 사람마다 다르다.";
        List<String> splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(3);
        ...
        splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(3);
        ...
        splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(2);
    ...
    @Test
    public void testJyo() {
        String text = "아무래도 그땐 그랬었죠 많이 힘들었으니까요 근데 이제는 괜찮아요 친구들이 많아졌어요 그때만 힘들었던거죠 이젠 괜찮아요";
        List<String> splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(6);
    }

    @Test
    public void testEcEfCases() {
        String text = "국내에 판매하는 OEM 수입차의 판매량이 2017년까지 하락세를 보이다 지난해 반등했으며 수입차 대비 비중도 높아질 전망이다.";
        List<String> splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(1);
    ...
    public void testQuotes() {
        String text = "연비테스트를 진행하면서 들었던 의문점인 ‘트립 컴퓨터 정보'에 대한 신뢰도 문제였다. 3대의 차량 모두 연료를 더 이상 들어가지 않을 때 까지 가득 주유한 뒤 193km를 이동했다.";
        List<String> splitted = kss.splitSentences(text);
        Assertions.assertThat(splitted.size()).isEqualTo(2);
